...
We now turn to the central empirical claim of this paper: under strict OOD settings where evaluation categories are disjoint from training categories, EGA achieves the highest ANNS Recall@1 across all three benchmarks. Table~\ref{tab:ood_summary} summarizes the results at $K=1$, $\text{nprobe}=1$.

\begin{table}[t]
\centering
\caption{Out-of-distribution ANNS Recall@1 at $K=1$, nprobe=1. Bold marks the best result in each row. EGA achieves the highest recall across all three OOD benchmarks and is the only adapter that consistently outperforms the frozen CLIP baseline.}
\label{tab:ood_summary}
\small
\begin{tabular}{lcccc}
\hline
Dataset          & CLIP   & ICon   & SRL    & EGA (Ours) \\
\hline
CIFAR-10         & 0.617  & 0.788  & 0.831  & \textbf{0.858} \\
FGVC-Aircraft    & 0.773  & 0.875  & 0.863  & \textbf{0.898} \\
Food-101         & 0.842  & 0.811  & 0.827  & \textbf{0.886} \\
\hline
\end{tabular}
\end{table}
...
The empirical results reveal a stark contrast in spatial preservation capabilities, particularly given the strong pre-trained representations of common food items. Global contrastive and structural regularization methods, such as ICon and SRL, suffer performance degradation when confined to a lightweight adapter bottleneck. Specifically, at $K=1$ and nprobe=1, the ANNS Recall@1 for ICon and SRL drops to 0.811 and 0.827 respectively, falling below the frozen CLIP baseline of 0.842. This phenomenon indicates that global contrastive objectives aggressively distort the already well-structured latent manifold of unseen categories when the representation capacity is constrained. 
Conversely, EGA exhibits exceptional stability and generalization, achieving the highest ANNS Recall@1 of 0.886 under the exact same adapter constraints. By leveraging local geometric anchors rather than forcing global distributions, EGA successfully compresses class-specific clusters while maintaining the structural integrity of the broader semantic space, significantly enhancing retrieval efficiency even on a highly optimized baseline manifold.
...
training triplets contribute zero gradient at steady state. Across CIFAR-10, CIFAR-100, FGVC-Aircraft, and Food-101, EGA achieves the highest ANNS Recall@1 over both the frozen baseline and the strongest competing adapters, with the largest gains appearing exactly where global methods fail on strictly unseen categories.
