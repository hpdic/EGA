...
Vector similarity search is the core mechanism for retrieving high dimensional representations in modern applications. While foundational vision language models such as CLIP \cite{clip_icml21} offer rich zero shot semantic capabilities, their latent manifolds often exhibit a systemic disconnect between semantic alignment and geometric proximity. This geometric illusion means that true semantic neighbors frequently overlap with noise in the Euclidean space, rendering standard approximate nearest neighbor search inefficient \cite{savietto2026geometryrepresentationalfailuresvision}. 

To address these metric inconsistencies, recent approaches apply global contrastive objectives or structural regularizers \cite{alshammariunifying, dong2025improve} to recalibrate the representation space. However, these methods rely on batch wide alignment and uniformity pressures that generate dense gradient fields during training. When applied through lightweight adapters for parameter efficient tuning, this global optimization aggressively reconfigures the entire latent manifold. Consequently, the delicate geometric structures of unseen fine grained categories are systematically distorted, causing out of distribution retrieval performance to collapse below that of the frozen baseline.

To overcome this fundamental limitation, we propose Euclidean Geodesic Alignment (EGA), a parameter efficient manifold smoothing approach. EGA introduces a residual adapter equipped with zero initialization and L2 normalization constraints to refine the pre trained semantic space. Instead of forcing global uniformity, EGA optimizes local geometric anchors via a small margin triplet loss. This localized objective compresses class specific clusters and resolves metric misalignment without subjecting the broader pre trained semantic relations to destructive global gradients.

Extensive empirical evaluations across CIFAR-10, CIFAR-100, FGVC-Aircraft, and Food-101 validate the superiority of our method. EGA converges to a sparse gradient regime where 96.5 percent of training triplets produce zero gradient at steady state, effectively preserving the structural integrity of unseen categories. As a result, EGA consistently outperforms state of the art global contrastive methods in both in distribution and out of distribution settings. For instance, on strictly unseen fine grained domains, competing methods degrade below the frozen baseline, whereas EGA significantly elevates label precision by up to 18.5 percent and achieves parity with multi probe search baselines using only single cell lookups.
...
\begin{itemize}
    ...
    \item We propose Euclidean Geodesic Alignment (EGA), a parameter efficient adapter leveraging residual structures and local geometric anchors to smooth manifold irregularities without destroying pre trained semantic relations.
    \item We demonstrate through rigorous empirical evaluation that EGA significantly accelerates approximate nearest neighbor search and exhibits superior out of distribution generalization on complex fine grained domains, outperforming contemporary global contrastive objectives.
\end{itemize}
...
In this paper, we introduced Euclidean Geodesic Alignment (EGA) to bridge the gap between semantic alignment and geometric proximity in foundational representation spaces. We identified that global contrastive objectives often produce dense gradient fields that distort the latent manifold of unseen categories. To resolve this, EGA utilizes a lightweight residual adapter initialized as an identity mapping and trained with a small margin triplet loss. This localized approach enables the model to refine specific class clusters while preserving the broader semantic integrity established by the pre trained backbone.

Our experimental results across multiple benchmarks demonstrate that EGA significantly improves label precision and retrieval recall, particularly in out of distribution settings. The analysis of gradient sparsity reveals that EGA converges to a regime where only a small fraction of training samples exert update pressure, effectively protecting the manifolds of unseen categories from destructive optimization. This finding suggests that sparse local updates are vital for maintaining the zero shot capabilities of foundational models during adaptation. While our current evaluation focuses on visual representations, future work could explore the application of EGA to more diverse multimodal manifolds. By achieving parity with multi probe search through single cell lookups, EGA proves that correcting the metric geometry at the source is a highly efficient strategy for scalable vector retrieval. Unlike conventional structural regularization methods that rely on global optimization pressures, EGA provides a sparse and targeted solution for robust manifold alignment in vector search systems.
