...
% ─────────────────────────────────────────────────────────────────────────────
% Section 1: Introduction (consolidated, Framing A)
% Replace the entire \section{Introduction} block with the content below.
% ─────────────────────────────────────────────────────────────────────────────
...
(OOD) regime, the empirical consequence is striking: adapters trained with
state-of-the-art global objectives such as ICon~\cite{alshammariunifying} and
...
training trades semantic correctness for geometric indexability.
...
\begin{itemize}
    ...
    \item \textbf{Mechanism and evaluation.} We provide a mechanistic
    explanation grounded in gradient sparsity: EGA's triplet objective converges
    to a regime where $96.5\%$ of training triplets contribute zero gradient,
    leaving unseen class regions structurally unchanged from CLIP. We validate
    this mechanism empirically and demonstrate through evaluations on three OOD
    benchmarks (CIFAR-10, FGVC-Aircraft, Food-101) that ICon and SRL fall below
    the frozen CLIP baseline in Label Precision on every dataset---catastrophically
    so on Food-101, where they degrade by $32$--$36$ percentage points---while
    EGA is the only adapter that improves both Label Precision and ANNS Recall
    over the frozen baseline on the easier OOD settings, with bounded
    degradation under heavy OOD shift.
\end{itemize}
...
% ─────────────────────────────────────────────────────────────────────────────
% Section 4.3: Out-of-Distribution Generalization (consolidated, Framing A)
% Replace the entire \subsection{Out-of-Distribution Generalization} block
...
\vspace{-1em}
\centering
\caption{OOD performance at $K=1$, $\text{nprobe}=1$. Bold $=$ exceeds CLIP;
underline $=$ below CLIP.}
