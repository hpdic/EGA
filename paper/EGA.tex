...
\vspace{-1em}
\centering
\caption{OOD performance at $K=1$, nprobe=1. Bold $=$ exceeds CLIP; underline $=$ below CLIP.}
\label{tab:ood_summary}
\small
...
\hline
\multirow{2}{*}{CIFAR-10}
  & AR@1 & 0.863 & \textbf{0.913} & \textbf{0.942} & \textbf{0.986} \\
  & LP@1 & 0.880 & \underline{0.841} & \underline{0.794} & \textbf{0.886} \\
\hline
\multirow{2}{*}{Aircraft}
  & AR@1 & 0.773 & \textbf{0.875} & \textbf{0.863} & \textbf{0.898} \\
  & LP@1 & 0.512 & \underline{0.423} & \underline{0.441} & \textbf{0.548} \\
\hline
\multirow{2}{*}{Food-101}
  & AR@1 & 0.842 & \underline{0.811} & \underline{0.827} & \textbf{0.886} \\
  & LP@1 & 0.881 & \underline{0.557} & \underline{0.520} & \underline{0.798} \\
\hline
\end{tabular}
\vspace{-1em}
\end{wraptable}
We evaluate OOD generalization on three benchmarks: CIFAR-10 (trained on CIFAR-100), FGVC-Aircraft (80 seen / 20 unseen classes), and Food-101 (80 seen / 21 unseen classes). Table~\ref{tab:ood_summary} summarizes the headline result. The two metrics tell different stories. On ANNS Recall, all adapters generally improve over the frozen baseline, indicating that adapter-trained embeddings are easier to index with IVF---this is the geometric notion of retrieval quality. On Label Precision, however, the picture inverts: ICon and SRL fall below the frozen CLIP baseline on every OOD dataset, with degradation reaching $-32$ to $-36$ percentage points on Food-101. This contrast reveals that global contrastive adaptation buys geometric indexability at the cost of semantic correctness---an adapter can make retrieval faster while making the retrieved neighbors less relevant. EGA, in contrast, simultaneously improves both metrics on CIFAR-10 and Aircraft. On the more challenging Food-101 (where 21 unseen classes share substantial visual overlap with the 80 seen classes), EGA retains the strongest Label Precision among all adapters with a bounded $-8$ pp degradation, while still leading on ANNS Recall.


\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig/cifar10_generalization_recall.pdf}
    \caption{ANNS Recall@$K$ versus nprobe on CIFAR-10. All adapters improve geometric retrieval over the frozen CLIP baseline, with EGA achieving the largest gain.}
    \label{fig:generalization_recall}
\end{figure}

To validate the tradeoff in Table~\ref{tab:ood_summary} under fine-grained domain shifts, we present per-dataset retrieval curves on FGVC-Aircraft (20 unseen classes) and Food-101 (21 unseen classes). For a fair comparison, all methods train identical lightweight adapters on top of frozen CLIP features and differ only in their loss functions.

\paragraph{The geometric story (ANNS Recall).}
Across all three OOD benchmarks, every adapter---including ICon and SRL---improves or matches the frozen CLIP ANNS Recall@1 (Table~\ref{tab:ood_summary}). On Aircraft, ICon and SRL reach 0.875 and 0.863 respectively, both clearly above the frozen baseline of 0.773. On CIFAR-10, all three adapters substantially exceed CLIP. This confirms that adapter training does succeed at \emph{geometric} reorganization: the resulting embedding distribution is more amenable to IVF-based indexing.

\paragraph{The semantic story (Label Precision).}
The same adapters tell a strikingly different story when measured by Label Precision. On Aircraft at $K=1$, $\text{nprobe}=1$, ICon and SRL drop to $0.423$ and $0.441$---a $9$--$13$ percentage-point degradation \emph{below} the frozen CLIP baseline of $0.512$. On Food-101, the gap widens dramatically: ICon and SRL fall to $0.557$ and $0.520$ from a frozen baseline of $0.881$, a $32$--$36$ percentage-point collapse. The retrieved neighbors are geometrically closer in the IVF index, but they belong to the wrong semantic class. EGA breaks this pattern: it improves Label Precision on Aircraft ($+3.6$ pp) and CIFAR-10 ($+0.6$ pp), and on the most challenging Food-101 setting it retains the strongest Label Precision among all adapters with a bounded $-8.3$ pp degradation, well above the $-32$ to $-36$ pp gap of global contrastive methods.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig/aircraft_recall.pdf}
    \caption{ANNS Recall@$K$ versus nprobe on 20 unseen FGVC-Aircraft classes.}
    \label{fig:aircraft}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{fig/food_recall.pdf}
    \caption{ANNS Recall@$K$ versus nprobe on 21 unseen Food-101 classes.}
    \label{fig:food}
\end{figure}
...
% Left: Active triplet ratio decays to 3.5\% at convergence (96.5\% zero-gradient). Right: ICon/SRL fall below frozen CLIP baseline on unseen classes; EGA improves it.
}
\label{fig:mechanism}
\vspace{-1em}
\end{wrapfigure}
...
%     \caption{
%         Gradient sparsity as the mechanism behind EGA's OOD generalization.
...
We have shown that global contrastive adapter training harms OOD retrieval performance, pushing unseen-class accuracy below the frozen baseline. EGA resolves this fundamental tension by converging to a sparse-gradient regime in which 96.5\% of training triplets contribute zero gradient at steady state. Across CIFAR-10, CIFAR-100, FGVC-Aircraft, and Food-101, EGA achieves the highest ANNS Recall@1 on all benchmarks and is the only adapter that consistently outperforms the frozen CLIP baseline on unseen categories. These results suggest that gradient sparsity at convergence is a useful design principle for building retrieval adapters that remain robust under distribution shift.

\paragraph{Limitations.}
Our study centers on visual representations from CLIP. Extending EGA to text and multimodal models, exploring unsupervised variants, and understanding how overlap between seen and unseen categories affects manifold preservation present avenues for future investigation.
