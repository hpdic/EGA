...
\vspace{-1em}
\centering
\caption{Out-of-distribution ANNS Recall@1 at $K=1$, nprobe=1. Bold marks the best result.}
\label{tab:ood_summary}
\small
\begin{tabular}{lcccc}
\hline
Dataset          & CLIP   & ICon   & SRL    & EGA \\
\hline
CIFAR-10         & 0.617  & 0.788  & 0.831  & \textbf{0.858} \\
FGVC-Aircraft    & 0.773  & 0.875  & 0.863  & \textbf{0.898} \\
Food-101         & 0.842  & 0.811  & 0.827  & \textbf{0.886} \\
...
\vspace{-1em}
\end{wraptable}
The generalization capabilities of EGA are evaluated on the CIFAR-10 dataset, providing a rigorous out-of-distribution (OOD) test since all models were trained exclusively on CIFAR-100. Figure~\ref{fig:generalization_recall} illustrates the recall performance across varying nprobe values and K settings. EGA consistently maintains a dominant lead over all baselines, including recent state-of-the-art methods like ICon~\cite{alshammariunifying} and SRL~\cite{dong2025improve}. At a minimum search budget of nprobe=1, EGA achieves a Recall@1 of 0.858, significantly outperforming SRL (0.831) and ICon (0.788). As shown in the inset zooms, EGA continues to hold a clear advantage even at nprobe=10. A key observation is the efficiency gain: EGA achieves a recall level at nprobe=5 that matches or exceeds the performance of supervised baselines at nprobe=10, indicating that the manifold smoothing provided by EGA effectively doubles the search efficiency in unseen data environments.
...
To further validate the robustness of EGA under fine-grained domain shifts, we extend our evaluation to the FGVC-Aircraft and Food-101 datasets. In both settings, models are trained on known categories and tested on strictly unseen categories (20 unseen aircraft classes and 21 unseen food classes, respectively). For a fair comparison, all baseline methods are restricted to training identical lightweight MLP adapters on top of frozen CLIP features.

The empirical results reveal a consistent pattern. Global contrastive methods (ICon and SRL) suffer severe performance degradation when confined to a lightweight adapter bottleneck. On FGVC-Aircraft, their Recall@1 collapses to 0.422 and 0.440 respectively—well below the frozen CLIP baseline of 0.773. On Food-101, they drop to 0.811 and 0.827, falling below the frozen baseline of 0.842. This indicates that global contrastive objectives aggressively distort the latent manifold of unseen fine-grained categories when the representation capacity is constrained.

In contrast, EGA exhibits exceptional stability and generalization on both datasets. It achieves the highest Recall@1 of 0.898 on FGVC-Aircraft and 0.886 on Food-101 under the exact same adapter constraints. By leveraging local geometric anchors rather than forcing global distributions, EGA successfully compresses class-specific clusters while maintaining the structural integrity of the broader semantic space, significantly enhancing retrieval efficiency even on highly optimized baseline manifolds.
