...
def train_lora_triplet(train_feats, train_labels, device, dim, epochs=150, batch_size=256, margin=0.2, rank=8):
...
def train_lora_infonce(train_feats, train_labels, device, dim, epochs=150, batch_size=256, rank=8):
...
def main():
    ...
    parser.add_argument("--rank", type=int, default=8, help="LoRA rank (default 8, use 64/128 for capacity-matched)")
...
# scripts/train_eval_lora.py
# Copyright (c) 2026 Dongfang Zhao <dzhao@uw.edu>
# Modified for capacity-matched LoRA (to address reviewer W7)
...
def train_lora_triplet(train_feats, train_labels, device, dim, epochs=150,
                       batch_size=256, margin=0.2, rank=8):
...
def run_for_dataset(name, feat_path, label_path, device, rank, epochs=150):
    ...
    # Use class split for Aircraft and Food-101
    if "cifar10" in feat_path.lower():
        # CIFAR-10 is cross-dataset OOD
        c100_feat = os.path.expanduser('~/hpdic/EGA/embeddings/cifar100_vit_b32_features.npy')
        c100_label = os.path.expanduser('~/hpdic/EGA/embeddings/cifar100_vit_b32_labels.npy')
        train_feats = np.load(c100_feat).astype(np.float32)
        train_feats = train_feats / np.linalg.norm(train_feats, axis=1, keepdims=True)
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--rank", type=int, default=8, help="LoRA rank (8=original, 64/128=capacity-matched)")
    parser.add_argument("--epochs", type=int, default=150)
    args = parser.parse_args()
    ...
    for name, feat_file, label_file in datasets:
        ...
        run_for_dataset(name, feat_path, label_path, device, rank=args.rank, epochs=args.epochs)

if __name__ == '__main__':
    main()

# (venv) (base) cc@uc-a100:~/hpdic/EGA$ python scripts/15_train_eval_lora.py --rank 8
# Device: cuda | LoRA rank: 8


# ======================================================================
#   Dataset: FGVC-Aircraft  (LoRA rank = 8)
# ======================================================================
#   [A] Training LoRA + Triplet ...
#     epoch 30/150  (rank=8)
#     epoch 60/150  (rank=8)
#     epoch 90/150  (rank=8)
#     epoch 120/150  (rank=8)
#     epoch 150/150  (rank=8)
# WARNING clustering 501 points to 100 centroids: please provide at least 3900 training points
#     LP@1 = 0.5655  AR@1 = 0.6488
#   [B] Training LoRA + InfoNCE ...
#     epoch 30/150  (rank=8)
#     epoch 60/150  (rank=8)
#     epoch 90/150  (rank=8)
...
#     epoch 150/150  (rank=8)
# WARNING clustering 501 points to 100 centroids: please provide at least 3900 training points
#     LP@1 = 0.4940  AR@1 = 0.5952

# ======================================================================
#   Dataset: Food-101  (LoRA rank = 8)
# ======================================================================
#   [A] Training LoRA + Triplet ...
#     epoch 30/150  (rank=8)
#     epoch 60/150  (rank=8)
...
#     epoch 120/150  (rank=8)
#     epoch 150/150  (rank=8)
#     LP@1 = 0.8812  AR@1 = 0.6687

# ======================================================================
#   Dataset: CIFAR-10  (LoRA rank = 8)
# ======================================================================
#   [A] Training LoRA + Triplet ...
#     epoch 30/150  (rank=8)
#     epoch 60/150  (rank=8)
...
#     epoch 120/150  (rank=8)
#     epoch 150/150  (rank=8)
#     LP@1 = 0.8660  AR@1 = 0.5996
# (venv) (base) cc@uc-a100:~/hpdic/EGA$ 



# (venv) (base) cc@uc-a100:~/hpdic/EGA$ python scripts/15_train_eval_lora.py --rank 64
...
# (venv) (base) cc@uc-a100:~/hpdic/EGA$ python scripts/15_train_eval_lora.py --rank 128
