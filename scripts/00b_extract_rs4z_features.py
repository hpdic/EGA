...
def main():
    ...
    # CIFAR-10 (test split = 10,000 samples)
    extract_from_hf('cifar10_vit_b32', 'uoft-cs/cifar10', 'test', model, preprocess, device, embed_dir, image_key='img', label_key='label')

    # FGVC Aircraft (test split = 3,333 samples)
    air_dataset = datasets.FGVCAircraft(root=data_dir, split='test', download=True)
    extract_from_torchvision('aircraft_test_vit_b32', air_dataset, model, preprocess, device, embed_dir)

    # Food-101 (validation split = 25,250 samples)
    extract_from_hf('food101', 'ethz/food101', 'validation', model, preprocess, device, embed_dir, image_key='image', label_key='label')

    print('All required RS4z dataset features are ready!')
